...
instance Eq a => Eq (List a) where
  ...
  (==) Empty _     = False
  (==) _ Empty     = False
  (==) (LNode val1 list1) (LNode val2 list2)
    | val1 /= val2 = False
...
-- instance (Price a, Foldable f) => Price (f a) where
--   price = foldr (\x acc -> price x + acc) 0
...
instance Eq RationalNumber where
  (RationalNumber n1 d1) == (RationalNumber n2 d2) = n1 * d2 == d1 * n2
...
simplify (RationalNumber n d) =
  RationalNumber (n `div` g) (d `div` g)
    where g = gcd n d
