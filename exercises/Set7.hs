...
member y (Set xs) = y `elem` xs

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add y (Set xs)
  | y `member` Set xs = Set xs
  | otherwise   = Set (sort (y:xs))

-- add val (Set xs)
--   | val `elem` xs = Set xs
--   | otherwise     = Set (takeWhile (<val) xs ++ [val] ++ dropWhile (<=val) xs)
...
data State = Start | EggsAdded | FlourAdded | SugarAdded | AllAdded | Mixed | Error | Finished
  deriving (Eq,Show)
...
step FlourAdded AddSugar = AllAdded
step SugarAdded AddFlour = AllAdded
step AllAdded Mix = Mixed
...
passwordAllowed pass (MinimumLength l) = length pass >= l
passwordAllowed pass (ContainsSome string) = intersect pass string /= ""
passwordAllowed pass (DoesNotContain string) = intersect pass string == ""
passwordAllowed pass (And a b) = passwordAllowed pass a && passwordAllowed pass b
passwordAllowed pass (Or a b) = passwordAllowed pass a || passwordAllowed pass b
...
render (Multiply x y) = "(" ++ render x ++ "*" ++ render y ++ ")"
