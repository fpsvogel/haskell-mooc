...
-- danger pos queens = any (\q -> sameRow q pos || sameCol q pos || sameDiag q pos || sameAntidiag q pos) queens

danger pos [] = False
danger pos (x:xs) = danger' pos x || danger pos xs

danger' :: Coord -> Coord -> Bool
danger' pos x
  | sameRow pos x = True
  | sameCol pos x = True
  | sameDiag pos x = True
...
--         | i == r && j == c = 'Q' : helper (nextCol (i, j)) qs
--         | i > n = ""
--         | j > n = '\n' : helper (nextRow (i, j)) ((r, c) : qs)
--         | danger' (i, j) (r, c) = '#' : helper (nextCol (i, j)) qs
--         | otherwise = '.' : helper (nextCol (i, j)) ((r, c) : qs)
--    in helper (1, 1) (sort qs)
